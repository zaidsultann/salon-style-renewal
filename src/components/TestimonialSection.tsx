...
const TestimonialSection = () => {
  ...
  return (
    <section className="py-12 md:py-20 bg-gradient-to-r from-rose-50 to-amber-50">
          ...
          >
            <CarouselContent className="-ml-2 md:-ml-4">
              {testimonials.map((testimonial, index) => (
                <CarouselItem key={index} className="pl-2 md:pl-4 basis-11/12 sm:basis-1/2 lg:basis-1/3">
                  <div className="bg-white p-4 md:p-6 rounded-lg shadow-lg h-full">
                    ...
                    <p className="text-gray-600 mb-4 italic text-sm md:text-base">"{testimonial.text}"</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
            {locations.map((location, index) => (
              <div key={index} className="bg-white p-4 md:p-6 rounded-lg shadow-lg">
                <h4 className="font-semibold text-gray-900 mb-2 text-sm md:text-base">{location.name}</h4>
                <p className="text-gray-600 text-xs md:text-sm mb-4">{location.address}</p>
                <a
                  href={location.reviewUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 bg-rose-600 hover:bg-rose-700 text-white font-semibold px-4 md:px-6 py-2 md:py-3 rounded-full transition-colors duration-300 text-sm md:text-base"
                >
